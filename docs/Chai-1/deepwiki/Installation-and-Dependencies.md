...
The `chai-lab` package requires Python 3.10 or higher, as specified in the project configuration [pyproject.toml L13](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/pyproject.toml#L13-L13)

 This minimum version requirement ensures compatibility with modern type annotations and language features used throughout the codebase.
...
Sources: [pyproject.toml L13](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/pyproject.toml#L13-L13)

 [requirements.in L31](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/requirements.in#L31-L31)
...
```
git clone https://github.com/chaidiscovery/chai-lab.gitcd chai-labpip install -e .
```

The build system is configured to automatically install in editable mode when possible.

**Command Line Interface:**
After installation, the `chai-lab` command becomes available in your environment, provided by the `chai_lab.main:cli` entry point [pyproject.toml L73](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/pyproject.toml#L73-L73)

Sources: [pyproject.toml L1](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/pyproject.toml#L1-L1)

 [pyproject.toml L72-L73](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/pyproject.toml#L72-L73)
...
Antipickle["antipickle ==0.2.0"]
Gemmi["gemmi ~0.7.5"]
ModelCIF["modelcif >=1.0"]
RDKit["rdkit ==2025.09.6"]
Biopython["biopython >=1.83"]
TMTools["tmtools >=0.0.3"]
Typer["typer ~0.12"]
Tqdm["tqdm ~4.66"]
...
Einops["einops ~0.8"]
Numba["numba >=0.59"]
JaxTyping["jaxtyping >=0.2.25"]
Beartype["beartype >=0.18"]
Pandera["pandera >=0.24"]

Pandas --> Pandera

subgraph subGraph3 ["Type Safety & Validation"]
    JaxTyping
    Beartype
    Pandera
    JaxTyping --> Beartype
...
subgraph subGraph0 ["Core Computing & DL"]
    Python
    Torch
    Numpy
    Pandas
...
subgraph subGraph2 ["Molecular Data Stack"]
    Gemmi
    ModelCIF
    RDKit
    Biopython
...
subgraph subGraph1 ["CLI & Utilities"]
    Typer
    Tqdm
    Matplotlib
end
...
| --- | --- | --- |
| `torch` | >=2.3.1 | Deep learning framework for model execution [requirements.in L31](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/requirements.in#L31-L31) |
| `rdkit` | 2025.09.6 | Chemical informatics; version pinned for typing consistency [requirements.in L14-L15](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/requirements.in#L14-L15) |
| `gemmi` | ~0.7.5 | PDB/mmCIF parsing and structure manipulation [requirements.in L13](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/requirements.in#L13-L13) |
| `antipickle` | 0.2.0 | Used to save/load heterogeneous Python structures [requirements.in L17](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/requirements.in#L17-L17) |
| `jaxtyping` | >=0.2.25 | Tensor shape and type annotations for runtime checking [requirements.in L29](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/requirements.in#L29-L29) |
| `beartype` | >=0.18 | Compatible typechecker for `jaxtyping` [requirements.in L30](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/requirements.in#L30-L30) |
...
InitPy["chai_lab/init.py"]
Hatchling["hatchling >=1.20"]
HatchReq["hatch-requirements-txt"]
VerRes["Dynamic Version Resolver"]
DepRes["Dynamic Dependency Resolver"]
Build["hatch build"]
Wheel[".whl file"]
SDist[".tar.gz sdist"]
...
subgraph subGraph2 ["Dynamic Resolution"]
    VerRes
    DepRes
end

subgraph subGraph1 ["Hatch Build Backend"]
    Hatchling
    HatchReq
end
...
**Package Exclusions:**
The build system excludes development, CI/CD, and large data directories from source distributions (`sdist`):
...
Sources: [Dockerfile.chailab L1-L85](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/Dockerfile.chailab#L1-L85)
 ...
 [devcontainer.json L1-L43](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/devcontainer.json#L1-L43)
...
Sources: [.github/workflows/ruff.yml L1-L29](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/.github/workflows/ruff.yml#L1-L29)

 [.github/workflows/mypy.yml L1-L30](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/.github/workflows/mypy.yml#L1-L30)

 [.github/workflows/publish-to-pypi.yml L1-L36](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/.github/workflows/publish-to-pypi.yml#L1-L36)

## Troubleshooting

### Dependency Resolution

If you encounter issues with `torch` or `nvidia` dependencies during local development, refer to the `mypy.yml` workflow which demonstrates how to install requirements while filtering specific packages:
`cat requirements.in | grep -v nvidia | grep -v torch` [mypy.yml L25](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/mypy.yml#L25-L25)
...
Sources: [requirements.in L14-L15](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/requirements.in#L14-L15)

 [mypy.yml L25](https://github.com/chaidiscovery/chai-lab/blob/66c38d1f/mypy.yml#L25-L25)