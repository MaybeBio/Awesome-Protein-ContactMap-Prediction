...
This page provides a high-level introduction to OpenFold, its architecture, and its capabilities. For detailed setup instructions, see [Installation and Environment Setup](/aqlaboratory/openfold/2-installation-and-environment-setup). For running predictions, see [Running Inference](/aqlaboratory/openfold/3-running-inference). For training details, see [Training OpenFold](/aqlaboratory/openfold/4-training-openfold).
...
**Sources:** [README.md L1-L57](https://github.com/aqlaboratory/openfold/blob/56da08ec/README.md?plain=1#L1-L57)
...
| --- | --- | --- | --- |
| **Monomer** | Single-chain protein prediction | `model_1` through `model_5` | Template-based or template-free, pTM scoring |
| **Multimer** | Multi-chain complex prediction | `model_*_multimer_v3` | Chain pairing, interface prediction, AlphaFold-Multimer v2.3 |
| **SoloSeq** | MSA-free single sequence prediction | `seq_model_esm1b_ptm` | ESM-1b embeddings, fast inference without MSA generation |
...
**Sources:** [docs/source/Inference.md L1-L195](https://github.com/aqlaboratory/openfold/blob/56da08ec/docs/source/Inference.md?plain=1#L1-L195)
 ...
 [docs/source/Single_Sequence_Inference.md L1-L57](https://github.com/aqlaboratory/openfold/blob/56da08ec/docs/source/Single_Sequence_Inference.md?plain=1#L1-L57)
...
**Sources:** [run_pretrained_openfold.py L1-L542](https://github.com/aqlaboratory/openfold/blob/56da08ec/run_pretrained_openfold.py#L1-L542)
 ...
 [openfold/data/data_pipeline.py L1-L1163](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/data/data_pipeline.py#L1-L1163)
...
**Sources:** [run_pretrained_openfold.py L397-L542](https://github.com/aqlaboratory/openfold/blob/56da08ec/run_pretrained_openfold.py#L397-L542)

 [scripts/utils.py L13-L66](https://github.com/aqlaboratory/openfold/blob/56da08ec/scripts/utils.py#L13-L66)
...
**Sources:** [openfold/data/data_pipeline.py L334-L563](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/data/data_pipeline.py#L334-L563)
 ...
 [openfold/data/data_pipeline.py L1006-L1163](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/data/data_pipeline.py#L1006-L1163)
...
**Sources:** [openfold/config.py L105-L283](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/config.py#L105-L283)

 [openfold/model/model.py L1-L500](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/model/model.py#L1-L500)
...
**Sources:** [train_openfold.py L1-L200](https://github.com/aqlaboratory/openfold/blob/56da08ec/train_openfold.py#L1-L200)

 [openfold/utils/loss.py L1-L500](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/utils/loss.py#L1-L500)
...
| --- | --- | --- | --- |
| `aatype` | `[N_res]` or `[N_res, 21]` | int32/float32 | Amino acid type (one-hot or indices) |
| `msa` | `[N_seq, N_res]` | int32 | Multiple sequence alignment |
| `deletion_matrix_int` | `[N_seq, N_res]` | int32 | Deletion counts in MSA |
| `template_aatype` | `[N_templ, N_res, 22]` | float32 | Template amino acid types |
| `template_all_atom_positions` | `[N_templ, N_res, 37, 3]` | float32 | Template atomic coordinates |
| `template_all_atom_mask` | `[N_templ, N_res, 37]` | float32 | Template atom validity mask |
| `residue_index` | `[N_res]` | int32 | Residue position indices |
| `seq_length` | `[N_res]` | int32 | Sequence length (repeated) |
| `all_atom_positions` | `[N_res, 37, 3]` | float32 | Ground truth coordinates (training) |
| `all_atom_mask` | `[N_res, 37]` | float32 | Atom validity mask (training) |
...
**Sources:** [openfold/data/data_pipeline.py L111-L131](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/data/data_pipeline.py#L111-L131)
...
**Sources:** [openfold/config.py L1-L283](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/config.py#L1-L283)

 [run_pretrained_openfold.py L184-L202](https://github.com/aqlaboratory/openfold/blob/56da08ec/run_pretrained_openfold.py#L184-L202)
...
| --- | --- | --- | --- |
| **DeepSpeed Attention** | `--use_deepspeed_evoformer_attention` | 2-3x faster inference | General use |
| **FlashAttention** | `globals.use_flash=True` | Memory-efficient attention | Sequences < 1000 residues |
| **cuEquivariance kernels** | `--use_cuequivariance_attention` | 1.2-1.5x speedup | All sequence lengths |
| **TensorRT** | `--trt_mode=run` | Module-level optimization | Batch inference |
| **BF16 precision** | `--precision=bf16` | ~1.5x speedup vs TF32 | Ampere+ GPUs |
| **Low-memory attention** | `long_sequence_inference` | Reduced memory | Long sequences (>1000 residues) |
| **Template offloading** | `offload_templates=True` | Reduced memory | Many templates |
| **Gradient checkpointing** | Training default | Reduced memory | Training |
| **Model tracing** | `--trace_model` | Faster repeated inference | Batch jobs |

**Sources:** [docs/source/Inference.md L145-L194](https://github.com/aqlaboratory/openfold/blob/56da08ec/docs/source/Inference.md?plain=1#L145-L194)

 [run_pretrained_openfold.py L184-L196](https://github.com/aqlaboratory/openfold/blob/56da08ec/run_pretrained_openfold.py#L184-L196)
...
**Sources:** [run_pretrained_openfold.py L356-L395](https://github.com/aqlaboratory/openfold/blob/56da08ec/run_pretrained_openfold.py#L356-L395)

 [openfold/np/protein.py L1-L300](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/np/protein.py#L1-L300)
...
**Sources:** [README.md L4-L20](https://github.com/aqlaboratory/openfold/blob/56da08ec/README.md?plain=1#L4-L20)

 [openfold/utils/import_weights.py L1-L200](https://github.com/aqlaboratory/openfold/blob/56da08ec/openfold/utils/import_weights.py#L1-L200)

## Getting Started

To begin using OpenFold:

1. **Installation**: Follow [Installation and Environment Setup](/aqlaboratory/openfold/2-installation-and-environment-setup) to set up dependencies and download databases
2. **Inference**: See [Running Inference](/aqlaboratory/openfold/3-running-inference) for structure prediction workflows * [Monomer Inference](/aqlaboratory/openfold/3.2-monomer-inference) for single-chain predictions * [Multimer Inference](/aqlaboratory/openfold/3.3-multimer-inference) for protein complexes * [Single Sequence Inference](/aqlaboratory/openfold/3.4-single-sequence-(soloseq)-inference) for fast MSA-free predictions
3. **Training**: See [Training OpenFold](/aqlaboratory/openfold/4-training-openfold) to train models from scratch
4. **Model Details**: Explore [Model Architecture](/aqlaboratory/openfold/5-model-architecture) for component-level documentation
...
**Sources:** [README.md L1-L57](https://github.com/aqlaboratory/openfold/blob/56da08ec/README.md?plain=1#L1-L57)
 ...
 [docs/source/Inference.md L1-L50](https://github.com/aqlaboratory/openfold/blob/56da08ec/docs/source/Inference.md?plain=1#L1-L50)