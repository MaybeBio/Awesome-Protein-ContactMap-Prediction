...
| --- | --- | --- |
| **AtomArray** | A core data structure from the `biotite` library used throughout Protenix to store atomic coordinates and annotations (residue names, chain IDs, etc.). | [protenix/data/utils.py L24-L27](https://github.com/bytedance/Protenix/blob/c3bfc365/protenix/data/utils.py#L24-L27) |
| **Bioassembly** | The functional form of a biological molecule, often consisting of multiple chains (asymmetric units) arranged by symmetry operations. | [scripts/prepare_training_data.py L29-L34](https://github.com/bytedance/Protenix/blob/c3bfc365/scripts/prepare_training_data.py#L29-L34) |
| **CCD (Chemical Component Dictionary)** | A reference dictionary containing standard geometric and chemical information for residues and ligands. | [protenix/data/core/ccd.py L1-L20](https://github.com/bytedance/Protenix/blob/c3bfc365/protenix/data/core/ccd.py#L1-L20) |
| **MSA (Multiple Sequence Alignment)** | A collection of evolutionarily related sequences used as input to provide co-evolutionary signals for structure prediction. | [runner/msa_search.py L1-L30](https://github.com/bytedance/Protenix/blob/c3bfc365/runner/msa_search.py#L1-L30) |
| **Polymer Entity** | A molecular chain consisting of repeating units (amino acids for proteins, nucleotides for DNA/RNA). | [protenix/data/core/parser.py L66-L67](https://github.com/bytedance/Protenix/blob/c3bfc365/protenix/data/core/parser.py#L66-L67) |
| **Asym ID (label_asym_id)** | A unique identifier for each distinct chain in a molecular structure. | [protenix/data/utils.py L112-L126](https://github.com/bytedance/Protenix/blob/c3bfc365/protenix/data/utils.py#L112-L126) |
...
A["Input JSON (SMILES/Sequence)"]
B["json_to_feature.py"]
C["SampleDictToFeatures Class"]
D["Tokenizes into TokenArray"]
E["Generates Chemical Features"]
F["Calculates Distances/Bonds"]
...
subgraph subGraph0 ["Natural Language / Input Space"]
    A
end
```
...
* **Fused Kernels:** Custom Triton-based GPU operations that combine multiple steps (e.g., Dropout + Residual Add) into a single kernel to reduce memory bandwidth overhead. * *Implementation:* `dropout_add_rowwise` in [protenix/model/modules/fused_ops.py L190-L212](https://github.com/bytedance/Protenix/blob/c3bfc365/protenix/model/modules/fused_ops.py#L190-L212)
* **Row-wise Mask Sharing:** An optimization where the dropout mask is shared across the token dimension to improve consistency and reduce random number generation overhead. * *Implementation:* `_dropout_add_rowwise_fwd_kernel` in [protenix/model/modules/fused_ops.py L41-L58](https://github.com/bytedance/Protenix/blob/c3bfc365/protenix/model/modules/fused_ops.py#L41-L58)
* **Inplace Operations:** Modifying tensors in-place to save memory, specifically utilized in the Pairformer blocks during inference. * *Pointer:* `inplace_safe` flag in [protenix/model/modules/pairformer.py L138-L169](https://github.com/bytedance/Protenix/blob/c3bfc365/protenix/model/modules/pairformer.py#L138-L169)
...
* **Refinement (mu):** Directly shifting the predicted clean coordinates ($x_0$) based on the potential gradient.
...
S["Updated Coords for Step i+1"]
E["TFGEngine.step()"]
P["Calculates dE/dx"]
G["_energy_and_grad()"]
M["Monte-Carlo Log-Prob Estimation"]

E --> G
M --> S

subgraph subGraph1 ["Guidance Logic"]
    P
    G
    M
    G --> P
